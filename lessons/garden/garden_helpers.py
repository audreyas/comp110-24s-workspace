...
# want tto get: {"flower": ["marigold", "zinnia"], "vegetable": ["carrots"]}
...
def lookup_by_kind_and_date(plants_by_kind: dict[str, list[str]], plants_by_date: dict[str, list[str]], kind: str, month: str) -> str:
    ...
    #Get a list of all plants of the specific kind and input 
    ...
    # "<kind>s to plant in <month>: <combined_lists>"
    if len(combined_list) > 0:
        return f"{kind}s to print in {month}: {combined_list}"
